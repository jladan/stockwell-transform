function [comps, aps, files] = compression_tests(data, ps, wname, measures, fileN)
%COMPRESSION_TESTS runs a series of thresholdings on the data (in the transformed space)
%
%   [comps, aps, files] = compression_tests(data, ps, wname, measures, fileN)
...
%
%   [comps, aps, images] = compression_tests(data, ps, wname, measures)
...
if nargin < 5
    ...
    files = {};
    aps = [];
    [tdata, s] = wavedec2(data, log2(min(size(data))), wname);
    for p = ps
        [cdata,ap] = compress(tdata,p);
        aps = [aps;ap];
        imData = waverec2(cdata,s,wname);
