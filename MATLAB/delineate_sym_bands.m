...
height = height + .5;
for p = 1:nx
    h(p) = line([bstart,bstart],[-0.5,height],properties{:});
    bstart = bstart + bx(p);
end
h(nx+1) = line([bstart,bstart],[-0.5,height],properties{:});

if M
    width = width + .5;
    bstart = 0.5;
    for p = 1:ny
        h(p+nx+1) = line([-0.5,width],[bstart,bstart],properties{:});
        bstart = bstart + by(p);
    end
    h(nx+ny+2) = line([-0.5,width],[bstart,bstart],properties{:});
else
    h = h(1:end-1);
end
