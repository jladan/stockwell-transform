...
function S = fdost_original(h)
    ...
    for j=1:num
        ...
        if v<0
            % This extra ramp amounts to a shift for negative frequencies
            % It corrects the phases to match the slow transform
            r2 = exp(tau*2*pi*i/b);
            S(band_start+(0:b-1)) = r2.*ramp.*tmp(tau+1);
        else
