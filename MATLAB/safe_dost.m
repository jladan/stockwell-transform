% A "safe" (ie. naive) implementation of the DOST as a bunch of inner products.

% input:  h = signal (row of length 2^n)
% output: S1 = actual DOST 
%               calculated from the basis vectors defined in 
%               eq. 14 of Stockwell's paper
%         S2 = Kind of DOST
%               calculated from analyitic sum (eq. 6)
%               Not actually correct, since the analytic sum has some special cases
...
error(nargchk(1,1,bargin));

    % first, get the various parameters.
    N = length(h);
    [vs,bs] = bands(N);     % params. of partitions of the frequency domain
    num = length(vs);
